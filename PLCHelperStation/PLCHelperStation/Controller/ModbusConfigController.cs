    ...
    public class ModbusConfigController : ControllerBase
    {
        ...
        public Result AddModbusConfug(ModbusConfig modbus)
        {
            using (var ctx = new TestDbContext())
            {
                ...
                else
                {
                    ...
                    _logger.LogWarning("已将新配置保存到数据库！");
                    return resultOk;
                }
        ...
        public ActionResult<List<ModbusConfig>> GetModbusConfug()
        {
            using (var ctx = new TestDbContext())
            {
                ...
                _logger.LogWarning("获取一次所有Modbus配置");
        ...
        public Result DeleteModbusConfig(int id)
        {
            using (var ctx = new TestDbContext())
            {
                ...
                if (plc != null)
                {
                    ...
                    _logger.LogWarning("执行删除Modbus配置操作，删除成功！");
                    return result;
                }
                else
                {
                    _logger.LogError("执行删除Modbus配置操作，不存在该对象！");
            ...
            try
            {
                using (var ctx = new TestDbContext())
                {
                    ...
                    else
                    {
                        ...
                        _logger.LogError("修改Modbus配置操作,修改失败：要修改的对象不存在");
                        return resultwarn;
                    }
                }
            }
                ...
                using (var ctx = new TestDbContext())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            _logger.LogWarning($"启用Modbus配置，id为：{Id} 的配置启用成功！");
                            return result;
                        }
                ...
                using (var ctx = new TestDbContext())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            _logger.LogWarning($"停用Modbus配置，id为：{Id} 的配置停用成功！");
                            return result;
                        }
