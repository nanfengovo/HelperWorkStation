    ...
    <div class="S7Card">
        ...
        <el-form :model="form">
                        <el-form-item label = "配置名称:">
                            <el-input v-model="form.S7Name" aria-placeholder="请输入配置名称" clearable  />
                        </el-form-item>
...
const update = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/ModbusConfig/UpdateS7Config?id=${form.value.id}&S7Name=${form.value.S7Name}&IP=${form.value.IP}&Port=${form.value.Port}&CPUType=${form.value.CPUType}&Rack=${form.value.Rack}&Slot=${form.value.Slot}`, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (response.data.code == 200) {
            // 显示成功消息
