...
//编辑
const Edit = async (index: number, row: { id: number }) => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/S7Config/UpdateS7Config?id=1&S7Name=TEST&IP=127.0.0.1&Port=102&CPUType=S7-1200&Rack=0&Slot=0`);
    } catch (error: any) {
        ElMessage({
            message: error.message,
            type: 'error',
