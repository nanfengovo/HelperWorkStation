...
const update = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/ModbusConfig/UpdateModbusConfig?id=${form.value.id}&ip=${form.value.ip}&port=${form.value.port}&name=${form.value.name}`, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (response.data.code == 200) {
            // 显示成功消息
