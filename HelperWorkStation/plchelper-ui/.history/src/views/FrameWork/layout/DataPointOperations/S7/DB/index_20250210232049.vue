...
const DeleteDBPointConfig = (id: number) => {
    ElMessageBox.confirm(
    '确定要删除此配置?',
    '警告',
    {
    ...
    .then(() => {
        //发送请求
        axios.delete(`http://localhost:5264/api/DBPointConfig/DeleteDBPointConfig?id=${id}`)
        .then((res) => {
            console.log(res.data);
            if (res.data.code == 200) {
                ElMessage({
...
//UpdateDBPointConfig按钮点击事件
const UpdateDBPointConfig = (id: number) => {
    ElMessageBox.confirm(
    '确定要更新此配置?',
    '警告',
    {
