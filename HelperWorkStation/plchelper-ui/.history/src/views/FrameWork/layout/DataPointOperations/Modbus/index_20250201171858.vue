...
const update = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/ModbusConfig/UpdateModbusConfig?Id=${form.value.id}&PLCName=${encodeURIComponent(form.value.plcName)}&SlaveId=${form.value.slaveId}&FunctionCode=${form.value.functioncode}&StartAddr=${form.value.address}&Num=${form.value.num}&ConfigName=${form.value.modbusname}`, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (response.data.code == 200) {
            // 显示成功消息
