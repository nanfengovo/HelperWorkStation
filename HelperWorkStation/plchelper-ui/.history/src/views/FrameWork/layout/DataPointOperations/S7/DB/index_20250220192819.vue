<template>
    ...
    <el-dialog v-model="diaDBPoint" :title=remarktitle width="500" draggable>
        ...
        <div class="dialog-footer">
            ...
            <el-button type="primary" @click="WriteDB">
            确认写入
            </el-button>
        </div>
        </template>
...
//写入变量点
let diaDBPoint = ref(false); //是否显示写入变量值对话框
let DBPointRead = ref(''); //读取变量值
const DBWrite = ref(''); //确认写入

const Write = async (index: number,row: { id: number, s7Name: string, dbName: string, dbType: string, dbAddress: string, dbOffset: string, remark: string, isOpen: boolean}) => {
    form.value = { ...row }; //复制数据
    ...
    if (response.data.code == 200) {
        DBPointRead.value = response.data.data; // 获取后端返回的数据数组
        ElMessage({
        message: "监控S7数据点成功!!",
        type: 'success',
        });
...
const WriteDB = async () => {
    try {
        const response = await axios.post(`http://127.0.0.1:5264/api/S7ReadAndWrite/WriteDBPoint?id=${form.value.id}&target=${DBWrite.value}`);
        ...
        if (response.data.code == 200) {
            ...
            DBWrite.value = '';
    const response = await axios.get(`http://127.0.0.1:5264/api/S7ReadAndWrite/ReadDBPoint?id=${form.value.id}`);
    if (response.data.code == 200) {
        DBPointRead.value = response.data.data; // 获取后端返回的数据数组
        ElMessage({
        message: "更新S7数据点监控成功!!",
