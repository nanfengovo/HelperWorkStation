            ...
            <el-table :data="ModbusData" stripe style="width: 100%" >
                ...
                <el-table-column label="操作" width="400">
                    <template #default="scope">
                        <el-button type="success" @click="Start(scope.$index, scope.row)">启用</el-button>
...
const form = ref<{id?:number, plcName: string,slaveId: number,functioncode:string,address:string, num:number, modbusname: string}>({id:0, plcName: '',slaveId: 1,functioncode:'',address:'', num:1, modbusname: ''});
...
const update = async () => {
    try {
        const url = `http://127.0.0.1:5264/api/ModbusConfig/UpdateModbusConfig?` +
            `Id=${encodeURIComponent(form.value.id)}` +
            `&PLCName=${encodeURIComponent(form.value.plcName)}` +
            `&SlaveId=${encodeURIComponent(form.value.slaveId)}` +
            `&FunctionCode=${encodeURIComponent(form.value.functioncode)}` +
            `&StartAddr=${encodeURIComponent(form.value.address)}` +
            `&Num=${encodeURIComponent(form.value.num)}` +
            `&ConfigName=${encodeURIComponent(form.value.modbusname)}`;
