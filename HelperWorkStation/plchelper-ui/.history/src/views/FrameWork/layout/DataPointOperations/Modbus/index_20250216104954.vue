<template>
    <div class="modbus">
        ...
        <!-- 插槽-编辑对话框 -->
        <div class="dialog">
            <el-dialog v-model="editFormVisible"  width="500" draggable class="dialogform" >
...
const WatchStatus = async (index: number, row: { id: number, plcName: string,slaveId: number,functionCode:string,startAddress:string, num:number, modbusName: string ,isOpen:boolean}) => {
  ...
  // 设置新的定时器，每隔1秒调用一次
    ...
    try {
    const response = await axios.get(`http://127.0.0.1:5264/api/ModbusReadAndWrite/ReadModbus?id=${row.id}`);
    if (response.data.code == 200) {
