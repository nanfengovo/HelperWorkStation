                ...
                <el-form :model="S7Config" class="S7Config-form">
                    <div class="content">
                        <el-form-item label = "S7配置名称:">
                            <el-input style="width: 180px;" v-model="S7Name" placeholder="请输入保存的配置名称"  />
                        </el-form-item>
...
const Delete = async (index: number, row: { id: number }) => {
    ...
    .then(async () => {
        try {
        const response = await axios.delete(`http://127.0.0.1:5264/api/ModbusConfig/DeleteS7Config?id=${row.id}`);
        if (response.data.code == 200) {
            // 显示成功消息
            ElMessage({
