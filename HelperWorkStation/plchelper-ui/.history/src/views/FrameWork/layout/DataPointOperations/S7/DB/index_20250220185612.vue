<template>
    ...
    <el-dialog v-model="diaDBPoint" :title=remarktitle width="500" draggable>
        ...
        <el-input
            v-model="DBPointWrite"
            style="width: 240px"
        />
        <template #footer>
        <div class="dialog-footer">
            <el-button @click="diaDBPoint = false">取消</el-button>
            <el-button type="primary" @click="WriteDBPoint">
            确认写入
            </el-button>
        </div>
        </template>
...
const Write = async (index: number,row: { id: number, s7Name: string, dbName: string, dbType: string, dbAddress: string, dbOffset: string, remark: string, isOpen: boolean}) => {
    ...
    //确认写入
    const DBPointWrite = ref('');
    const WriteDBPoint = async () => {
        try {
            const response = await axios.post(`http://'127.0.0.1:5264/api/S7ReadAndWrite/WriteDBPoint?id=${row.id}&value=${DBPointWrite.value}`);
            if (response.data.code == 200) {
                ElMessage({
                message: "写入S7数据点成功!!",
                type: 'success',
