...
const update = async () => {
    try {
        const data = {
            Id: form.value.id,
            PLCName: form.value.plcName,
            SlaveId: form.value.slaveId,
            FunctionCode: form.value.functioncode,
            StartAddr: form.value.address,
            Num: form.value.num,
            ConfigName: form.value.modbusname
        };

        const response = await axios.put(`http://127.0.0.1:5264/api/ModbusConfig/UpdateModbusConfig`, data, {
