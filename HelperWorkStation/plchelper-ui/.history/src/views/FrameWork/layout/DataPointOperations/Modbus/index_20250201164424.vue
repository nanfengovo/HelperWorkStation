...
const form = ref<{id?:number, plcName: string,slaveId: number,functioncode:string,address:string, num:number, modbusname: string}>({id:0, plcName: '',slaveId: 1,functioncode:'',address:'', num:10, modbusname: ''});
...
const update = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/ModbusConfig/UpdateModbusConfig?Id=${form.value.id}&PLCName=${form.value.plcName}&port=${form.value.port}&name=${form.value.name}`, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (response.data.code == 200) {
            // 显示成功消息
