<template>
    <div class="modbus">
        ...
        <el-card style="width: 100%" shadow="always" class="Modbus-card" >
         ...
         <!-- 插槽-编辑对话框 -->
         <el-dialog v-model="editFormVisible" title="修改Modbus配置" width="500" >
        <el-form :model="form">
            <el-form-item label = "选择PLC:">
                            <el-select 
                            v-model="selectedPLC" 
        ...
        <el-dialog title="修改ModbusConfig配置" v-model="dialogVisible" width=500>
            <el-form :model="editFormVisible" class="modbusConfig-form">
                    <div class="content">
                        <el-form-item label = "选择PLC:">
                            <el-select 
                            v-model="selectedPLC" 
                    ...
                    <div class="btn-group">
                        <el-button @click="cancel">取消此配置</el-button>
                        <el-button type="primary" @click = "UpdateMobusConfig">添加此配置</el-button>
                    </div>
                </el-form>
        </el-dialog>
    </div>
...
//编辑
//编辑
const Edit = (index: number, row: { id: number, ip: string, port: number, name: string}) => {
    form.value = { ...row };
    editFormVisible.value = true;
}
// const Edit = (index: number, row: { id: number, modbusName: string, plcName: string, name: string, SlaveId: number, functionCode: string, startAddress: string, num: number }) => {
//     editFormVisible.value = { ...row };
