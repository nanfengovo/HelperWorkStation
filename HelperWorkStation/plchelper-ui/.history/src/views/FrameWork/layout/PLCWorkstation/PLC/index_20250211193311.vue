<template>
    <div class="PLCCard">
        ...
        <!-- 插槽-编辑对话框 -->
        <el-dialog v-model="editFormVisible" title="修改PLC配置" width="500" draggable>
        <el-form :model="form">
...
const form = ref<{ id?: number, ip: string, port: number,name: string}>({ ip: '', port: 0 ,name:''});
...
//取消
const cancel = () => {
    if(inputIP.value === '' && inputPort.value === '' && inputName.value === ''){
        ElMessage({
        message: '还没填写任何内容',
        type: 'info',
    })
...
const update = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:5264/api/PLCConfig/UpdatePLCConfig?id=${form.value.id}&ip=${form.value.ip}&port=${form.value.port}&name=${form.value.name}`, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (response.data.code == 200) {
            // 显示成功消息
